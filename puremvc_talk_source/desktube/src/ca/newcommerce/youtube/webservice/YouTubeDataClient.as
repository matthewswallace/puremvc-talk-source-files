package ca.newcommerce.youtube.webservice 
{
	...
	*/
	...
	public class YouTubeDataClient extends EventDispatcher
	{
		...
		public function complaint(videoId:String, type:String, comment:String):Number
		{
		...
		protected function doComplaint(result:Object):void
		{
			...
			if (httpCode == "201")
			{
				dispatchEvent(new YouTubeEvent(YouTubeEvent.COMPLAINT_SUCCESSFUL, wrapper));
			}
			else if (httpCode == "400")
			{
				wrapper.error = content;
				dispatchEvent(new YouTubeEvent(YouTubeEvent.COMPLAINT_FAILED, wrapper));
			}
		}
		...
		protected function doComment(result:Object):void
		{
			...
			if (httpCode == "201")
			{
				var idEx:RegExp = /feeds\/api\/videos\/.+\/comments\/(.+)[<]\/id[>]/i;
				var id:String = idEx.exec(content)[1];
				
				wrapper.commentId = id;
				dispatchEvent(new YouTubeEvent(YouTubeEvent.COMMENT_SUCCESSFUL, wrapper));
			}
			else if (httpCode == "400")
			{
				wrapper.error = content;
				dispatchEvent(new YouTubeEvent(YouTubeEvent.COMMENT_FAILED, wrapper ));
			}
		}
